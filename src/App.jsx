...
import SnuzzLoader from "./components/common/SnuzzLoader";
...
export default function SnuzzLanding() {
  ...
  /* ── Loader: wait for fonts + a minimum display time ── */
  useEffect(() => {
    ...
    Promise.all([minDisplay, fontsReady]).then(() => setAppReady(true));
  }, []);
  ...
  /* ── APK download handler — v1.0 live ── */
  const handleDownload = () => {
    const a = document.createElement("a");
    a.href = "/snuzz.apk";
    a.download = "Snuzz-v1.0.apk";
    a.click();
  };

  if (!appReady) return <SnuzzLoader />;
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .dl-btn {
          ...
          background: #1A2718; color: #F4EFE4;
          font-family: 'DM Mono', monospace; font-size: 13px; font-weight: 500;
          letter-spacing: 0.08em; text-transform: uppercase;
          padding: 14px 28px; border: none; transition: 0.22s; text-decoration: none;
        }
        .dl-btn:hover { background: #5C7A5F; transform: translateY(-2px); }

        .ghost-btn {
          display: inline-flex; align-items: center; gap: 10px;
          background: transparent; color: #1A2718;
          font-family: 'DM Mono', monospace; font-size: 13px; font-weight: 400;
          letter-spacing: 0.08em; text-transform: uppercase;
          padding: 13px 24px; border: 1.5px solid rgba(26,39,24,0.25); transition: 0.22s; text-decoration: none;
        }
        .ghost-btn:hover { background: #1A2718; color: #F4EFE4; border-color: #1A2718; }
        ...
        .feat-row {
          display: grid; grid-template-columns: 160px 1px 1fr 148px; gap: 0 36px;
          padding: 26px 0; align-items: start;
          border-bottom: 1px solid rgba(26,39,24,0.13);
        ...
        /* Navbar links */
        .nav-lnk {
          color: rgba(26,39,24,0.45); font-size: 10px;
        ...
        .pixel-9-frame {
          ...
          padding: 10px;
          box-shadow:
            16px 32px 80px rgba(26,39,24,0.2),
